    ...
    <div class="max-w-md mx-auto bg-white p-5 rounded-lg shadow-lg">
        ...
        <form method="POST" action="{{ route('user.profile.update') }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input id="username" type="text" class="form-input mt-1 block w-full border border-gray-300 rounded-lg @error('username') border-red-500 @enderror" name="username" value="{{ Auth::guard('user')->user()->username }}" required autocomplete="username" autofocus>
                @error('username')
            ...
            <div class="mb-4">
                ...
                <input id="password" type="password" class="form-input mt-1 block w-full border border-gray-300 rounded-lg @error('password') border-red-500 @enderror" name="password" autocomplete="new-password">
                @error('password')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input id="password-confirm" type="password" class="form-input mt-1 block w-full border border-gray-300 rounded-lg" name="password_confirmation" autocomplete="new-password">
            </div>

            <div class="mb-4">
                <label for="profile_picture" class="block text-gray-700">{{ __('Profile Picture') }}</label>
                <input id="profile_picture" type="file" class="form-input mt-1 block w-full border border-gray-300 rounded-lg" name="profile_picture">
            </div>
